...
const { mirrowProcessor } = require('../../../services/processors/mirrow/mirrowProcessor');
...
const mapRowToMirrowSchema = (row, month, year, filename) => ({
    year: parseInt(year),
    ...
    gst_rate: safeNum(row['GST Rate']),
    cgst: safeNum(row['CGST']),
    sgst: safeNum(row['SGST']),
    igst: safeNum(row['IGST']),
    total_amount: safeNum(row['Total Amount']),
...
const computeSummary = (rows) => {
    ...
    rows.forEach(row => {
        qty     += safeNum(row['Quantity'] ?? row.quantity ?? 0);
...
const computePivotSummary = (rows) => {
    let qty = 0, taxable = 0, igst = 0, cgst = 0, sgst = 0;
    rows.forEach(row => {
        qty     += safeNum(row['Sum of Quantity'] ?? 0);
...
const generatePreview = async (req, res, next) => {
    try {
        ...
        if (!req.file && (!req.files || !req.files.file)) {
            return res.status(400).json({ error: 'Mirrow raw report file is required' });
        }
        const fileBuffer = req.file ? req.file.buffer : req.files.file[0].buffer;

        const processedData = await mirrowProcessor(
            fileBuffer,
        ...
        const taskId = uuidv4();
        const filename = `mirrow_${brand.name}_${month}_${year}_${taskId}.xlsx`;
        const processPath = path.join(OUTPUT_DIR, filename);

        const dbRows = processedData.workingData.map(row =>
            mapRowToMirrowSchema(row, month, year, filename)
        );
        ...
        console.error('Mirrow Preview Error:', error);
        next(error);
    }
};
...
const generateCommit = async (req, res, next) => {
    try {
        ...
        res.json({
            success: true,
            message: 'Mirrow file generated and saved successfully',
            data: { filename: processFile, count: finalData.length }
        });
    } catch (error) {
        console.error('Mirrow Commit Error:', error);
        next(error);
    }
};
...
const generateDiscard = async (req, res, next) => {
    try {
        ...
        console.error('Mirrow Discard Error:', error);
        next(error);
    }
};
...
const generate = async (req, res, next) => {
    try {
        ...
        await ensureDir();
        const id = uuidv4();
        const filename = `mirrow_${brand.name}_${month}_${year}_${id}.xlsx`;
        ...
        res.json({
            success: true,
            message: 'Mirrow working file generated successfully',
            data: { filename, count: dbRows.length }
        });
    } catch (error) {
        console.error('Mirrow Generation Error:', error);
        next(error);
    }
};
